...
class ServicePageSpec extends UnitSpec with FakeApplicationBuilder {
  ...
  override def beforeEach(): Unit =
    ...
    serviceEndpoint(GET, "/api/repositories/service-1/module-dependencies?version=latest&flag=latest",willRespondWith = (200, Some("[]")))
    serviceEndpoint(GET, "/api/repositories/service-1/module-dependencies?version=0.0.1&flag=production", willRespondWith = (200, Some("[]")))
    serviceEndpoint(GET, "/api/repositories/service-1/module-dependencies?version=0.0.1&flag=qa", willRespondWith = (200, Some("[]")))
  ...
  case class ServicePageBehaviour(serviceName: String, repoName: String) {
    trait Setup {
      if (serviceName == repoName)
        ...
        serviceEndpoint(GET, s"/api/repositories/$repoName/module-dependencies?version=0.0.1&flag=production" , willRespondWith = (200, Some("[]")))
        serviceEndpoint(GET, s"/api/repositories/$repoName/module-dependencies?version=0.0.1&flag=qa" , willRespondWith = (200, Some("[]")))
        serviceEndpoint(GET, s"/api/repositories/$repoName/module-dependencies?version=latest&flag=latest", willRespondWith = (200, Some("[]")))
      }
    }
