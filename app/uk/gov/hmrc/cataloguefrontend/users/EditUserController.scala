...
class EditUserController @Inject()(
  ...
  def editUserLanding(username: UserName, organisation: Option[String]): Action[AnyContent] =
    auth.authenticatedAction(
      ...
      implicit request =>
      userManagementConnector.getUserAccess(username).map: accessOpt =>
        accessOpt.fold{
  ...
  def editUserAccess(username: UserName, organisation: Option[String]): Action[AnyContent] =
    auth.authenticatedAction(
      ...
      implicit request =>
      (for
        existingTooling <- EitherT(userManagementConnector.getUserAccess(username).map:
                             case Some(access) => Right(access)
                             case None         => Left(NotFound(error_404_template()))
                           )
