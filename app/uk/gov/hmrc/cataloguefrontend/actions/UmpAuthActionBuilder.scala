...
class UmpAuthActionBuilder @Inject() (
  ...
  private def withCheck(optGroup: Option[String]) =
    new ActionBuilder[UmpAuthenticatedRequest, AnyContent] with ActionRefiner[Request, UmpAuthenticatedRequest] {
      ...
      def refine[A](request: Request[A]): Future[Either[Result, UmpAuthenticatedRequest[A]]] = {
        ...
        val signInPage =
          appRoutes.AuthController.showSignInPage(targetUrl = Some(request.target.uriString).filter(_ => request.method == "GET"))

        def fromSession(key: String) =
          EitherT.fromOption[Future](request.session.get(key), Redirect(signInPage))
        ...
        (for {
          ...
          user        <- EitherT.fromOptionF[Future, Result, User](userManagementAuthConnector.getUser(token), Redirect(signInPage))
