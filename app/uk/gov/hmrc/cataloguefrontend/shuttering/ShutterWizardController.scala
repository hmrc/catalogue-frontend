...
class ShutterWizardController @Inject() (
  ...
  def withGroup(continueUrl: => Call) =
    auth.authenticatedAction(
      ...
      //predicate   = Predicate.Permission(Resource.from("shutter-api", serviceName.asString), IAAction("SHUTTER"))
      //retrieval   =
    )
  ...
  def start(shutterType: ShutterType, env: Environment, serviceName: Option[String]) =
    withGroup(appRoutes.ShutterWizardController.start(shutterType, env, serviceName))
  ...
  def step1Get(serviceName: Option[String]) =
    withGroup(appRoutes.ShutterWizardController.step1Get(serviceName))
      .async { implicit request =>
        (for {
  ...
  def step1Post =
    withGroup(appRoutes.ShutterWizardController.step1Get(None))
      .async { implicit request =>
        (for {
          step0Out      <- getStep0Out
  ...
  def step2aGet =
    withGroup(appRoutes.ShutterWizardController.step2aGet)
      .async { implicit request =>
        (for {
          step0Out <- getStep0Out
          step1Out <- getStep1Out
          step2af  <- EitherT.liftF {
                        cacheRepo.getFromSession(step2aKey)
                          .map {
  ...
  def step2bGet =
    withGroup(appRoutes.ShutterWizardController.step2bGet)
      .async { implicit request =>
        (for {
          step1Out  <- getStep1Out
  ...
  def step3Get =
    withGroup(appRoutes.ShutterWizardController.step3Get)
      .async { implicit request =>
        (for {
  ...
  def step4Get =
    withGroup(appRoutes.ShutterWizardController.step4Get)
      .async { implicit request =>
        (for {
           step0Out     <- getStep0Out
           step1Out     <- getStep1Out
