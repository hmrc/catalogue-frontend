...
class ShutterWizardController @Inject() (
  ...
  def start(shutterType: ShutterType, env: Environment, serviceName: Option[String]) =
    withGroup.async { implicit request =>
      ...
      } yield Redirect(appRoutes.ShutterWizardController.step1Get(serviceName))
      ...
      (for {
        ...
        serviceNames <-
          EitherT
            ...
            .leftFlatMap(_ =>
              EitherT.left[NonEmptyList[String]](showPage1(step0Out.shutterType, step0Out.env, boundForm.withGlobalError(Messages("No services selected"))).map(BadRequest(_)))
            )
  ...
  def step2aPost =
    withGroup.async { implicit request =>
      ...
      } yield Redirect(appRoutes.ShutterWizardController.step2bGet)).merge
    }
  ...
  private def showPage2b(form: Form[Step2bForm], step1Out: Step1Out, step2aOut: Option[Step2aOut])(implicit request: Request[Any]): EitherT[Future, Result, Html] =
    for {
      step0Out <- getStep0Out
    ...
    } yield page2b(form2b, step0Out.env, step1Out, requiresOutageMessage, outageMessageSrc, defaultOutageMessage, outagePageStatus, back)
  ...
  def step2bPost =
    withGroup.async { implicit request =>
      ...
      } yield Redirect(appRoutes.ShutterWizardController.step3Get)).merge
    }
  ...
  private def showPage3(form3: Form[Step3Form], shutterType: ShutterType, env: Environment, step1Out: Step1Out, step2Out: Step2bOut)(implicit request: Request[Any]): Html = {
    val back =
  ...
  def step3Get =
    withGroup.async { implicit request =>
      ...
      } yield Ok(html)).getOrElse(Redirect(appRoutes.ShutterWizardController.step1Post))
    }
    ...
    withGroup.async { implicit request =>
      (for {
        ...
        _ <- step3Form.bindFromRequest
               .fold(
                 hasErrors = formWithErrors => EitherT.left(Future(showPage3(formWithErrors, step0Out.shutterType, step0Out.env, step1Out, step2bOut)).map(BadRequest(_))),
      ...
      } yield Redirect(appRoutes.ShutterWizardController.step4Get)).merge
    }
  ...
  def step4Get =
    withGroup.async { implicit request =>
      ...
      } yield Ok(html).withSession(request.session - sessionIdKey)).merge
    }
