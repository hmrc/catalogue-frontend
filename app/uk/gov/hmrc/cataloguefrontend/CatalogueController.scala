  ...
  def changePrototypePassword(repoName: String) =
    auth
      ...
      (for {
        ...
        _            = logger.info(s"User $user has triggered a password reset for prototype: $repoName which resulted in success = ${response.success} - ${response.message}")
  ...
  def setPrototypeStatus(repoName: String, status: PrototypeStatus): Action[AnyContent] =
    auth
      ...
      buildDeployApiConnector
        ...
        .map { response =>
          logger.info(s"Setting prototype $repoName to status ${status.displayString}, triggered by User: $user resulted in success = ${response.success} - ${response.message}")
          Redirect(routes.CatalogueController.repository(repoName))
        }
    }
