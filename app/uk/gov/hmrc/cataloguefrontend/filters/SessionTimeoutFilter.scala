...
class SessionTimeoutFilter @Inject()(
  ...
  val config = SessionTimeoutFilterConfig.fromConfig(configuration)
  ...
  override def apply(f: (RequestHeader) => Future[Result])(rh: RequestHeader): Future[Result] = {
    ...
    play.api.Logger(getClass).info(s"SessionTimeoutFilter: MDC: ${uk.gov.hmrc.play.http.logging.Mdc.mdcData}: ${rh.method} ${rh.uri}")
